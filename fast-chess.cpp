...
Bitboard getEmptySquares(Board * board) {
    return bnot(getOccupiedSquares(board));
}
...
Bitboard knightMoves(Bitboard moving_piece, Board * board, char color) {
    return knightAttacks(moving_piece) & bnot(getColoredPieces(board, color));
}
...
Bitboard kingAttacks(Bitboard moving_piece) {
    ...
    return kingAtks & bnot(moving_piece);
}

Bitboard kingMoves(Bitboard moving_piece, Board * board, char color) {
    return kingAttacks(moving_piece) & bnot(getColoredPieces(board, color));
}
...
Bitboard bishopMoves(Bitboard moving_piece, Board * board, char color) {
    return bishopAttacks(moving_piece, board, color) & bnot(getColoredPieces(board, color));
}
...
Bitboard rookMoves(Bitboard moving_piece, Board * board, char color) {
    return rookAttacks(moving_piece, board, color) & bnot(getColoredPieces(board, color));
}
...
void clearPositions(Board * board, Bitboard positions) {
    board->whiteKing = board->whiteKing & bnot(positions);
    board->whiteQueens = board->whiteQueens & bnot(positions);
    board->whiteRooks = board->whiteRooks & bnot(positions);
    board->whiteKnights = board->whiteKnights & bnot(positions);
    board->whiteBishops = board->whiteBishops & bnot(positions);
    board->whitePawns = board->whitePawns & bnot(positions);
    board->blackKing = board->blackKing & bnot(positions);
    board->blackQueens = board->blackQueens & bnot(positions);
    board->blackRooks = board->blackRooks & bnot(positions);
    board->blackKnights = board->blackKnights & bnot(positions);
    board->blackBishops = board->blackBishops & bnot(positions);
    board->blackPawns = board->blackPawns & bnot(positions);
}
...
int legalMoves(Move * legalMoves, Position * position, char color) {
  ...
  Move *pseudoMoves = new Move[MAX_BRANCHING_FACTOR];
...
int staticOrderLegalMoves(Move * orderedLegalMoves, Position * position, char color) {
    ...
    Position newPosition;
    Node *nodes = new Node[legalCount];
    Node *orderedNodes = new Node[legalCount];
...
int getCaptureSequence(Move * captures, Position * position, int targetSquare) {
    Move *allCaptures = new Move[MAX_BRANCHING_FACTOR];
    Move *targetCaptures = new Move[MAX_ATTACKING_PIECES];
    ...
    delete [] targetCaptures;

    return targetCount;
}

int staticExchangeEvaluation(Position * position, int targetSquare) {
    Move* captures = new Move[MAX_ATTACKING_PIECES];
...
int quiescenceEvaluation(Position * position) {
  ...
  Move *captures = new Move[MAX_BRANCHING_FACTOR];
...
Node staticSearch(Position * position) {
  ...
  for (i=0; i<moveCount; i++) {
    ...
    if (score == winScore(position->toMove)) {
      Node n = (Node) { .move = moves[i], .score = score };
      delete [] moves;
      return n;
    }
...
int alphaBetaNodes(Node * sortedNodes, Position * position, char depth) {
    Node *nodes = new Node[MAX_BRANCHING_FACTOR];
    Move *moves = new Move[MAX_BRANCHING_FACTOR];
    int moveCount = legalMoves(moves, position, position->toMove);
...
Node iterativeDeepeningAlphaBeta(Position * position, char depth, int alpha, int beta, BOOL verbose, uint8_t* percent) {
  ...
  for (i=0; i<moveCount; i++) {
    ...
    if (score == winScore(position->toMove)) {
      Node n = (Node) { .move = nodes[i].move, .score = score };
      delete [] nodes;
      return n;
    }
